...
async function runTests() {
  ...
  await fetch(`${API_BASE}/translate`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
    body: JSON.stringify({ query: 'space exploration', sourceLang: 'en' })
  });
  ...
  const esRes = await fetch(`${API_BASE}/translate`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
    body: JSON.stringify({ query: 'exploración espacial', sourceLang: 'es' })
  });
  ...
  if (!queries.includes('exploración espacial') || !queries.includes('space exploration')) {
  ...
  const eventId = esData.results?.[0]?._id;
  if (!eventId) throw new Error("No events returned to save");
